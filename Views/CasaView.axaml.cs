...
namespace Tarea.Views
{
    public partial class CasaView : UserControl
    {
        ...
        private void EditRoomTitle_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
        ...
        private void RoomTitleEdit_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Enter)
            {
                room.CommitTitleEditCommand.Execute(null);
                e.Handled = true;
            }
            else if (e.Key == Key.Escape)
            {
                room.CancelTitleEditCommand.Execute(null);
                e.Handled = true;
            }
        }
        ...
        private void ShowQuickAdd_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
                ...
                && DataContext is CasaViewModel vm)
            {
                ...
                // Walk up to find the card, then search for the quick add TextBox
                ...
                if (cardRoot != null)
                {
                    ...
                    {
                        var tb = FindQuickAddTextBox(cardRoot);
                        if (tb != null)
                        {
                            tb.Focus();
                            tb.SelectAll();
        ...
        private TextBox? FindQuickAddTextBox(Visual parent)
        {
            foreach (var child in parent.GetVisualChildren())
            {
                if (child is Border b && (b.Tag as string) == "QuickAddArea")
                {
                    var tb = ViewHelpers.FindVisualChild<TextBox>(b);
                    if (tb != null) return tb;
                }
                var result = FindQuickAddTextBox(child);
        ...
        private void AddRoom_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            else if (e.Key == Key.Escape)
            {
                ...
                BtnToggleAddRoom.IsVisible = true;
                e.Handled = true;
            }
        }
        ...
        {
            AddRoomPanel.IsVisible = true;
            BtnToggleAddRoom.IsVisible = false;
            Dispatcher.UIThread.Post(() =>
            {
                TxtAddRoom.Focus();
        ...
        private void ToggleAddRoom_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
            var showing = !AddRoomPanel.IsVisible;
            AddRoomPanel.IsVisible = showing;
            BtnToggleAddRoom.IsVisible = !showing;
